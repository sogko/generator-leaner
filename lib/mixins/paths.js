...
var PathsMixins = {
  ...
  resolvesNgAppRootPath: function (p) {
    if (!p) p = '';
    return path.join(this.paths.client.apps.root, this.ngAppName || '<undefined>', p);
  },
  resolvesNgModuleRootPath: function (p) {
    if (!p) p = '';
    return path.join(this.resolvesNgAppRootPath(), this.ngModuleName, p);
  },
  ...
  normalizeNgModuleName: function (val) {
    return this._.camelize(this._.slugify(this._.humanize(val)));
  },
